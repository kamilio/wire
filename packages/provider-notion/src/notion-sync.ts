...
export type NotionBlock = Readonly<{ type: string; content: string; properties: JsonObject; rich_text: NotionRichText; indent: number }>;
export type NotionTree = Readonly<{ id: string; block: JsonObject; children: readonly NotionTree[] }>;
export type NotionOperation = Readonly<{ pointer: JsonObject; path: readonly JsonValue[]; command: string; args: JsonValue }>;
export type NotionSidecarBlock = Readonly<{ id: string; path: readonly number[]; type: string; hash: string; snapshot: JsonObject }>;
export type NotionDiffSummary = Readonly<{ inserted: number; updated: number; deleted: number; moved: number }>;
type UserMentions = Readonly<Record<string, string>>;

function object(value: JsonValue): JsonObject { return value as JsonObject; }
function array(value: JsonValue): JsonValue[] { return value as JsonValue[]; }
function text(value: JsonValue): string { return value as string; }
function notionAuthError(): Error { return new Error("notion cookie authentication is missing or expired. Run `wire notion login` once; other commands reuse saved cookies."); }
function richTextText(value: JsonValue): string { return (value as JsonValue[][]).map((segment) => segment[0] as string).join(""); }
function titleRichText(value: string): NotionRichText { return value === "" ? [] : [[value]]; }
function pointer(id: string, spaceId: string): JsonObject { return { table: "block", id, spaceId }; }
function formatBlockId(id: string): string { return id.length === 32 ? `${id.slice(0, 8)}-${id.slice(8, 12)}-${id.slice(12, 16)}-${id.slice(16, 20)}-${id.slice(20)}` : id; }
function compact(value: JsonValue): string { return stableJsonCompact(value); }
function hash(value: JsonValue): string { return `sha256:${createHash("sha256").update(compact(value)).digest("hex")}`; }
...
function closingDelimiter(value: string, start: number, delimiter: string): number {
  for (let index = start; index < value.length; index += 1) if (!escaped(value, index) && value.startsWith(delimiter, index)) return index;
  return -1;
}

function markdownLink(value: string): { label: string; url: string; length: number } | null {
...
function normalizeMarks(marks: readonly JsonValue[]): JsonValue[] {
...
function normalizeRichText(value: JsonValue): JsonValue {
  ...
  for (const raw of value as JsonValue[][]) {
    ...
    if (content === "") continue;
    const marks = raw.length > 1 ? normalizeMarks(raw[1] as JsonValue[]) : [];
    ...
    const segment: JsonValue[] = marks.length === 0 ? [trimmedContent] : [trimmedContent, marks];
    if (trimmedContent === "") continue;
    const previous = output[output.length - 1];
    if (previous !== undefined && compact(previous[1] ?? []) === compact(segment[1] ?? [])) previous[0] = `${previous[0] as string}${trimmedContent}`;
...
function parseInline(value: string): NotionRichText {
  const segments: NotionRichText = [];
  let rest = value;
  const push = (content: string, marks: JsonValue[] = []) => { if (content !== "") segments.push(marks.length === 0 ? [content] : [content, marks]); };
  const pushMarked = (content: string, mark: JsonValue) => {
    for (const segment of parseInline(content)) {
      const marks = normalizeMarks([...((segment[1] as JsonValue[] | undefined) ?? []), mark]);
  ...
  while (rest.length > 0) {
    ...
    if (dateSpan !== null) {
      pushMarked(dateSpan[2]!, ["d", JSON.parse(unescapeHtml(dateSpan[1]!)) as JsonValue]);
    ...
    const patterns = [
      { match: /^`([^`]+)`/, mark: ["c"] as JsonValue },
      { match: /^<u>(.*?)<\/u>/, mark: ["_"] as JsonValue },
    ...
    for (const pattern of patterns) {
      ...
      if (pattern.mark !== null) {
        const mark = pattern.mark as JsonValue[];
        push(mark[0] === "c" ? match[1]! : unescapeInlineMarkdown(match[1]!), [pattern.mark]);
      }
      else push(unescapeInlineMarkdown(match[2]!), [["h", match[1]!]]);
...
function renderSegment(content: string, marks: readonly JsonValue[]): string {
  ...
  for (const raw of marks) {
    ...
    else if (mark[0] === "a") link = mark[1] as string;
    else if (mark[0] === "_") rendered = `<u>${rendered}</u>`;
    else if (mark[0] === "h") rendered = `<span data-notion-color="${escapeHtml(mark[1] as string)}">${rendered}</span>`;
    else if (mark[0] === "e") rendered = `<span data-notion-equation="${escapeHtml(mark[1] as string)}">${rendered}</span>`;
    else if (mark[0] === "p" || mark[0] === "u") rendered = `<span data-notion-mention="${mark[0] as string}" data-notion-id="${escapeHtml(mark[1] as string)}"${mark[0] === "p" ? ` data-notion-space-id="${escapeHtml(mark[2] as string)}"` : ""}>${rendered}</span>`;
...
function renderRichText(value: JsonValue): string {
  return (normalizeRichText(value) as JsonValue[][]).map((segment) => renderSegment(text(segment[0]!), (segment[1] as JsonValue[] | undefined) ?? [])).join("");
}

function renderUserMentions(markdownText: string, mentions: UserMentions): string {
  let output = markdownText;
  for (const [userId, handle] of Object.entries(mentions)) {
    output = output.replace(new RegExp(`<span data-notion-mention="u" data-notion-id="${escapeRegExp(escapeHtml(userId))}">(.*?)</span>`, "g"), (_match, content: string) => content.replaceAll("‣", `@${handle}`));
  }
  return output;
}

function hydrateUserMentions(markdownText: string, mentions: UserMentions): string {
  let output = markdownText;
  for (const [userId, handle] of Object.entries(mentions)) {
...
function baseUserMentions(base: JsonObject): UserMentions | undefined {
  return base["user_mentions"] as UserMentions | undefined;
}

function notionData(pageId: string, markdown: string, tree: NotionTree, mentions: UserMentions | undefined): JsonObject {
...
function lineIndent(line: string): number { return Math.floor(line.match(/^ */)![0].length / 2); }
function stripIndent(line: string): string { return line.replace(/^ +/, ""); }
...
function isTableSeparator(row: readonly string[]): boolean {
  return row.every((cell) => /^:?-{3,}:?$/.test(cell.trim()));
}

function markdownBlockStart(stripped: string): boolean {
...
function listContinuation(lines: readonly string[], index: number, baseIndent: number, indent: number, firstLine: string): Readonly<{ content: string; index: number }> {
...
function parseLines(lines: readonly string[], baseIndent = 0): NotionBlock[] {
  ...
  while (index < lines.length) {
    ...
    if (heading !== null) {
      ...
      const properties: Record<string, JsonValue> = stripped.endsWith("{toggle}") ? { format: { toggleable: true } } : {};
    ...
    if (stripped === ":::notion-opaque") {
      ...
      blocks.push({ type: snapshot["type"] as string, content: "", properties: { notion_opaque: snapshot }, rich_text: [], indent });
      continue;
    }
...
export function parseNotionMarkdown(markdown: string): readonly NotionBlock[] {
...
function parserBlockToNotionBlock(block: NotionBlock, id: string, parentId: string, parentTable: string, spaceId: string, userId: string, currentTime: number, columnOrder?: readonly string[]): JsonObject {
  ...
  if (["header", "sub_header", "sub_sub_header"].includes(block.type)) return { ...base, properties: { title: block.rich_text }, ...(block.properties["format"] === undefined ? {} : { format: block.properties["format"]! }) };
...
export function buildNotionCreateOperations(blocks: readonly NotionBlock[], pageId: string, spaceId: string, userId: string, currentTime: number, createId: () => string = () => randomUUID(), initialTableColumnOrder?: readonly string[]): Readonly<{ operations: readonly NotionOperation[]; topLevelIds: readonly string[] }> {
  ...
  for (const block of blocks) {
    ...
    if (block.type === "table_row") parentId = lastTableId!;
      ...
      if (parent === undefined) throw new Error(`Indented Notion Markdown block has no parent at indent ${block.indent - 1}`);
      parentId = parent;
    }
...
function canonicalBlock(value: JsonObject, columnOrder?: readonly string[]): JsonObject {
  const type = value["type"] as string;
  if (type === "text" && object(value["format"] ?? {})["toggleable"] === true) return { type: "toggle", title: normalizeRichText(object(value["properties"]!)["title"]!) };
  ...
  if (["header", "sub_header", "sub_sub_header"].includes(type)) return { type, title: normalizeRichText(object(value["properties"] ?? { title: [] })["title"] ?? []), toggleable: object(value["format"] ?? {})["toggleable"] === true };
  return { type, title: normalizeRichText(object(value["properties"] ?? { title: [] })["title"] ?? []) };
}

function canonicalParserBlock(block: NotionBlock, columnOrder?: readonly string[]): JsonObject {
  if (block.properties["notion_opaque"] !== undefined) return canonicalBlock(object(block.properties["notion_opaque"]!), columnOrder);
  if (block.type === "header" || block.type === "sub_header" || block.type === "sub_sub_header") return { type: block.type, title: normalizeRichText(block.rich_text), toggleable: object(block.properties["format"] ?? {})["toggleable"] === true };
...
export function notionBlockContentHash(block: NotionBlock | JsonObject, columnOrder?: readonly string[]): string {
  return hash("rich_text" in block ? canonicalParserBlock(block as NotionBlock, columnOrder) : canonicalBlock(block as JsonObject, columnOrder));
}

export function sidecarBlocksFromNotionTree(tree: NotionTree): readonly NotionSidecarBlock[] {
  const blocks: NotionSidecarBlock[] = [];
  const walk = (node: NotionTree, path: readonly number[], columnOrder?: readonly string[]) => {
    const type = node.block["type"] as string;
    const nextColumnOrder = type === "table" ? object(node.block["format"]!)["table_block_column_order"] as readonly string[] : columnOrder;
...
type LocalNotionNode = { block: NotionBlock; children: LocalNotionNode[] };

function localTree(blocks: readonly NotionBlock[]): LocalNotionNode[] {
  const roots: LocalNotionNode[] = [];
  const stack: Array<{ indent: number; children: LocalNotionNode[] }> = [{ indent: -1, children: roots }];
  for (const block of blocks) {
    const node: LocalNotionNode = { block, children: [] };
...
function emitUpdates(remote: JsonObject, local: NotionBlock, blockId: string, spaceId: string, columnOrder?: readonly string[]): NotionOperation[] {
  ...
  if (typeChanged) {
    ...
    for (const key of Object.keys(nextProperties)) if (compact(remoteProperties[key] ?? null) !== compact(nextProperties[key]!)) operations.push({ pointer: pointer(blockId, spaceId), path: ["properties", key], command: "set", args: nextProperties[key]! });
  }
  const nextFormat = object(args["format"] ?? {});
  if ((typeChanged || args["format"] !== undefined || remote["format"] !== undefined) && compact(remote["format"] ?? {}) !== compact(nextFormat)) operations.push({ pointer: pointer(blockId, spaceId), path: ["format"], command: "set", args: nextFormat });
  return operations;
}

function flattenLocalSubtree(node: LocalNotionNode): readonly NotionBlock[] {
  const blocks: NotionBlock[] = [];
  const walk = (value: LocalNotionNode, indent: number) => {
...
function positionRootCreateOperations(operations: readonly NotionOperation[], parentId: string, spaceId: string, previousId: string | undefined): readonly NotionOperation[] {
  return operations.map((operation) => {
    if (operation.pointer["id"] === parentId && operation.pointer["spaceId"] === spaceId && operation.path.length === 1 && operation.path[0] === "content") return { ...operation, command: previousId === undefined ? "listBefore" : "listAfter", args: previousId === undefined ? { id: object(operation.args)["id"]! } : { id: object(operation.args)["id"]!, after: previousId } };
    return operation;
  });
}

function localSubtreeSize(node: LocalNotionNode): number {
  return 1 + node.children.reduce((total, child) => total + localSubtreeSize(child), 0);
}

function remoteSubtreeSize(node: NotionTree): number {
  return 1 + node.children.reduce((total, child) => total + remoteSubtreeSize(child), 0);
}

function deleteRemoteSubtreeOperations(node: NotionTree, parentId: string, spaceId: string): readonly NotionOperation[] {
  return [
...
export function diffNotionBlockTrees(remoteTree: NotionTree, localBlocks: readonly NotionBlock[], _sidecarBlocks: readonly NotionSidecarBlock[], ambient: Readonly<{ spaceId: string; userId: string; currentTime: number }>): Readonly<{ operations: readonly NotionOperation[]; summary: NotionDiffSummary }> {
...
function renderNode(tree: NotionTree, indent: number, parentType = ""): string[] {
  const prefix = "  ".repeat(indent);
  const type = tree.block["type"] === "text" && object(tree.block["format"] ?? {})["toggleable"] === true ? "toggle" : tree.block["type"] as string;
  ...
  if (type === "equation") return [`${prefix}:::equation`, ...title.split("\n").map((line) => `${prefix}${line}`), `${prefix}:::`];
  if (type === "page") return [`${prefix}:::page`, `${prefix}${title}`, ...tree.children.flatMap((child) => renderNode(child, indent + 1, type)), `${prefix}:::`];
  if (type === "column_list") return [`${prefix}:::columns`, ...tree.children.flatMap((child) => renderNode(child, indent + 1, type)), `${prefix}:::`];
  if (type === "column") return [`${prefix}:::column`, ...(format["column_ratio"] === undefined ? [] : [`${prefix}${format["column_ratio"] as number}`]), ...tree.children.flatMap((child) => renderNode(child, indent + 1, type)), `${prefix}:::`];
  if (type === "transclusion_container") return [`${prefix}:::synced`, ...tree.children.flatMap((child) => renderNode(child, indent + 1, type)), `${prefix}:::`];
  if (type === "callout" || type === "toggle") return [`${prefix}:::${type}`, `${prefix}${title}`, ...tree.children.flatMap((child) => renderNode(child, indent + 1, type)), `${prefix}:::`];
  if (type === "table") {
    const columns = object(tree.block["format"]!)["table_block_column_order"] as string[];
  ...
  if (type !== "text") {
    ...
    if (tree.block["format"] !== undefined) return [`${prefix}:::notion-format`, `${prefix}:::format ${compact(tree.block["format"]!)}`, ...opaque, `${prefix}:::`];
    return opaque;
  }
...
export function renderNotionTreeToMarkdown(tree: NotionTree, mentions?: UserMentions): string {
  ...
  const body = tree.children.flatMap((child, index) => [
    ...(index === 0 || (tree.children[index - 1]!.block["type"] === child.block["type"] && compactTypes.has(child.block["type"] as string)) ? [] : [""]),
    ...renderNode(child, 0),
  ]);
...
async function notionPost(runtime: RuntimeCapabilities, path: string, cookie: string, body: JsonObject, headers: HeadersInit): Promise<JsonObject> {
...
async function fetchTree(runtime: RuntimeCapabilities, url: string, source: Source): Promise<Readonly<{ tree: NotionTree; userId: string; spaceId: string; cookie: string; headers: HeadersInit }>> {
  ...
  do {
    ...
    for (const [blockId, wrapper] of Object.entries(object(object(data["recordMap"]!)["block"]!))) blocks.set(blockId, object(object(object(wrapper)["value"]!)["value"]!));
    const cursors = data["cursors"] as readonly JsonObject[];
  ...
  while (true) {
    ...
    while (pending.length > 0) {
      ...
      for (const child of block["content"] as readonly string[] | undefined ?? []) pending.push(child);
    }
    ...
    for (const [index, result] of (data["results"] as readonly JsonObject[]).entries()) {
      const block = object(result["value"]!);
      blocks.set((block["id"] as string | undefined) ?? missingIds[index]!, block);
    }
  }
  const build = (idValue: string): NotionTree => {
    const block = blocks.get(idValue)!;
    if (idValue !== pageId && block["type"] === "page") return { id: idValue, block, children: [] };
    return { id: idValue, block, children: (block["content"] as readonly string[] | undefined ?? []).filter((child) => blocks.get(child)?.["alive"] !== false).map(build) };
  };
  return { tree: build(pageId), userId, spaceId, cookie, headers };
}

export async function fetchNotionDocument(runtime: RuntimeCapabilities, url: string, source: Source): Promise<FetchedDocument> {
...
export async function uploadNotionDocument(runtime: RuntimeCapabilities, markdown: string, _markdownPath: string): Promise<UploadedDocument> {
  ...
  const pageOperation: NotionOperation = {
  ...
  const pageTree: NotionTree = {
    id: pageId,
    block: pageOperation.args as JsonObject,
...
function splitTitle(markdown: string): Readonly<{ title: string; body: string }> {
...
export async function synchronizeNotionDocument(runtime: RuntimeCapabilities, url: string, base: JsonValue, markdown: string, _markdownPath: string): Promise<FetchedDocument> {
  ...
  const baseMarkdown = baseObject["markdown"] as string | undefined;
  ...
  const titleRich = split.title === "" ? object(remote.tree.block["properties"]!)["title"]! as NotionRichText : titleRichText(split.title);
  if (split.title !== "" && compact(object(remote.tree.block["properties"]!)["title"]!) !== compact(titleRich)) operations.unshift({ pointer: pointer(remote.tree.id, remote.spaceId), path: ["properties", "title"], command: "set", args: titleRich });
  if (operations.length > 0) await notionPost(runtime, "saveTransactionsFanout", remote.cookie, { requestId: randomUUID(), transactions: [{ id: randomUUID(), spaceId: remote.spaceId, operations }] }, remote.headers);
  const outputTree: NotionTree = { id: remote.tree.id, block: { ...remote.tree.block, properties: { ...object(remote.tree.block["properties"]!), title: titleRich } }, children: localTree(localBlocks).map((node, index) => outputNode(node, remote.tree.children[index], remote.tree.id, remote, ambient.currentTime)) };
  const output = renderNotionTreeToMarkdown(outputTree, mentions);
  return { title: output.split("\n")[0]!.replace(/^# */, ""), markdown: output, data: notionData(remote.tree.id, output, outputTree, mentions) };
}
