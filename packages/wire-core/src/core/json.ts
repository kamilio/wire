...
function serializeNumber(value: number): string {
  ...
  if (serialized.includes("e")) {
    return serialized.replace(/e([+-]?)(\d+)$/, (_, sign: string, exponent: string) => `e${sign === "" ? "+" : sign}${exponent.padStart(2, "0")}`);
  }
  const absolute = Math.abs(value);
  if (absolute !== 0 && absolute < 1e-4) {
    return value.toExponential().replace(/e([+-])(\d+)$/, (_, sign: string, exponent: string) => `e${sign}${exponent.padStart(2, "0")}`);
  }
  return serialized;
}

function serialize(value: JsonValue, indent: number | undefined, level: number): string {
...
export function stableJsonCompact(value: JsonValue): string {
  return serialize(value, undefined, 0);
}

export function stableJsonPretty(value: JsonValue): string {
