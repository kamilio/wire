function gidFromUrl(value) {
    ...
    if (project !== -1)
        return parts[project + 1];
    if (parts.at(-1) === "list")
        return parts.at(-2);
    return parts.at(-1) === "f" ? parts.at(-2) : parts.at(-1);
}
function taskUrl(projectGid, gid) {
...
function key(kind, gid, index) {
    return gid === null ? `new:${kind}:${index}` : `${kind}:${gid}`;
}
export function parseAsanaMarkdown(markdown) {
    const entities = [];
    let projectGid = "";
    let projectUrl = "";
    let section = null;
    ...
    for (const [lineIndex, line] of markdown.split("\n").entries()) {
        ...
        if (line.startsWith("# ")) {
            ...
            if (value.gid === null || projectGid !== "")
                throw new Error(`Invalid Asana project heading at line ${lineIndex + 1}.`);
            ...
            if (projectGid === "")
                throw new Error(`Asana section appears before the project at line ${lineIndex + 1}.`);
            ...
            if (section === null)
                throw new Error(`Asana milestone appears outside a section at line ${lineIndex + 1}.`);
            ...
            if (section === null)
                throw new Error(`Asana task appears outside a section at line ${lineIndex + 1}.`);
            ...
            if (task === null)
                throw new Error(`Asana subtask appears without a task at line ${lineIndex + 1}.`);
            const state = checkbox(line.slice(4));
            const value = linked(state.value);
            ...
            throw new Error(`Unsupported Asana Markdown at line ${lineIndex + 1}: ${line}`);
        const identity = entities.at(-1).gid;
        if (identity !== null) {
            if (identities.has(identity))
                throw new Error(`Duplicate Asana identity ${identity}.`);
            identities.add(identity);
        }
    }
    if (projectGid === "")
        throw new Error("Missing Asana project heading.");
    return { projectGid, projectUrl, entities };
}
export function renderAsanaMarkdown(document) {
...
export function asanaChanges(base, other) {
    const baseValues = values(base);
    const otherValues = values(other);
    const changes = [];
    for (const entity of other.entities) {
        if (entity.gid === null)
            changes.push({ operation: "create", key: entity.key, field: null, value: entity });
        else if (!baseValues.has(entity.key))
            changes.push({ operation: "create", key: entity.key, field: null, value: entity });
        else {
            const previous = baseValues.get(entity.key);
            for (const field of ["name", "completed", "parent", "section", "milestone"]) {
                if (previous[field] !== entity[field])
                    changes.push({ operation: "update", key: entity.key, field, value: entity[field] });
            }
        }
    }
    for (const entity of base.entities)
        if (entity.gid !== null && !otherValues.has(entity.key))
            changes.push({ operation: "delete", key: entity.key, field: null, value: entity });
    ...
    for (const group of groups) {
        ...
        for (const [index, entityKey] of after.entries())
            if (before[index] !== entityKey)
                changes.push({ operation: "update", key: entityKey, field: "order", value: index === 0 ? null : after[index - 1] });
    }
    return changes;
}
...
export function asanaConflicts(local, remote) {
    ...
    for (const change of local) {
        ...
        if (other !== undefined) {
            if (other.operation !== change.operation || JSON.stringify(other.value) !== JSON.stringify(change.value))
                conflicts.push(path);
        }
        else if ((change.operation === "delete" || change.operation === "create") && remoteEntities.has(change.key))
            conflicts.push(change.key);
        else if (remote.some((candidate) => candidate.key === change.key && candidate.operation === "delete"))
            conflicts.push(change.key);
    }
    return [...new Set(conflicts)].sort();
}
export function asanaDocument(value) {
    return value;
}
export function asanaSnapshot(document) {
    return document;
}
