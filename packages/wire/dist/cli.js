...
function normalizeDefaultCommandArgv(argv) {
...
function normalizeLeadingDebugTargetArgv(argv) {
    for (let index = 2; index < argv.length; index += 1) {
        ...
        if (arg === "--output") {
            ...
            if (value !== undefined && isDebugTarget(value))
                return [...argv.slice(0, index), value, ...argv.slice(index + 2), arg];
            return argv;
        }
        else if (arg.startsWith("--debug=")) {
            const value = arg.slice("--debug=".length);
            if (isDebugTarget(value))
                return [...argv.slice(0, index), value, ...argv.slice(index + 1), "--debug"];
            return argv;
        }
...
function normalizeVersionCommandArgv(argv) {
    const words = positionalWords(argv);
    if (words === null || words[0] !== "version")
        return argv;
...
function normalizeHelpCommandArgv(argv) {
    const separatorIndex = argv.indexOf("--", 2);
...
function normalizeFlagAliases(argv) {
    const normalized = [];
...
function normalizeBareDebugArgv(argv) {
    const normalized = [];
    for (let index = 0; index < argv.length; index += 1) {
        const arg = argv[index];
        if (index >= 2 && arg === "--") {
...
function isSourceLikeWithoutScheme(value) {
...
function expectedSourceMessage(prefix, value) {
...
function hasRootSeparator(argv) {
    return rootSeparatorIndex(argv) !== -1;
}
function rootSeparatorIndex(argv) {
    for (let index = 2; index < argv.length; index += 1) {
        const arg = argv[index];
        if (arg === "--")
            return index;
        if (arg === "--output" || (arg === "--debug" && argv[index + 1] !== undefined && debugModeNames.has(argv[index + 1])))
            index += 1;
        else if (arg.startsWith("--output=") || (arg.startsWith("--debug=") && debugModeNames.has(arg.slice("--debug=".length))))
            continue;
        else
            return -1;
    }
...
function argsBeforeSeparator(argv) {
    const args = argv.slice(2);
    const separator = args.indexOf("--");
    return separator === -1 ? args : args.slice(0, separator);
}
function lastValidOutputFormat(argv) {
...
function cliErrorText(message, usage) {
...
function renderCliError(argv, message, usage, displayMessage = message) {
    if (wantsJsonOutput(argv))
        process.stdout.write(`${JSON.stringify({ error: { message, usage } }, null, 2)}\n`);
    else
        process.stdout.write(cliErrorText(displayMessage, usage));
    process.exitCode = 1;
}
...
function excessPositionals(argv) {
    ...
    if (command in fixedCommandPositionals)
        return Object.freeze({ commandPath: command, expected: fixedCommandPositionals[command].expected, received: words.length - 1 });
    if (authServices.has(command)) {
        const authCommand = words[1];
        if (authCommand === undefined || !authCommands.has(authCommand))
            return null;
...
function missingPositionals(argv) {
    ...
    if (!(command in fixedCommandPositionals))
        return null;
    const positional = fixedCommandPositionals[command];
    if (words.length - 1 >= positional.expected || positional.expected === 0)
        return null;
    return Object.freeze({ commandPath: command, name: positional.name });
}
function renderMissingPositionals(argv) {
...
function renderExcessPositionals(argv) {
    const excess = excessPositionals(argv);
    if (excess === null || excess.received <= excess.expected)
        return false;
...
function renderRemovedCommand(argv) {
    if (hasRootSeparator(argv))
        return false;
    const words = positionalWords(argv);
    if (words === null || words.length === 0)
        return false;
    const replacement = removedCommandNames[words[0]];
    if (replacement === undefined)
        return false;
    renderCliError(argv, `Unknown command: ${words[0]}\n${replacement}`, "wire --help", `Unknown command: ${cliValue(words[0])}\n${replacement}`);
    return true;
}
function renderUnknownAuthCommand(argv) {
    if (hasRootSeparator(argv))
        return false;
    const words = positionalWords(argv);
    if (words === null || words.length < 2 || !authServices.has(words[0]) || authCommands.has(words[1]))
        return false;
    renderCliError(argv, `Unknown command: ${words[1]}`, `wire ${words[0]} --help`, `Unknown command: ${cliValue(words[1])}`);
    return true;
}
function authCommandAfterServiceSeparator(argv) {
    const words = [];
    for (let index = 2; index < argv.length; index += 1) {
        const arg = argv[index];
        if (arg === "--") {
        ...
        if (arg === "--output" || (arg === "--debug" && argv[index + 1] !== undefined && debugModeNames.has(argv[index + 1])))
            index += 1;
        else if (arg.startsWith("--output=") || arg.startsWith("--debug=") || arg.startsWith("-"))
            continue;
        else
            words.push(arg);
    }
...
function renderAuthCommandAfterServiceSeparator(argv) {
...
function renderInvalidSourceUrl(argv) {
    if (hasRootSeparator(argv))
        return false;
    const words = positionalWords(argv);
...
function renderSchemelessResourceUrl(argv) {
    if (hasRootSeparator(argv))
        return false;
    const words = positionalWords(argv);
...
function renderUnknownRootInput(argv) {
    const words = positionalWords(argv);
    if (words === null || words.length === 0)
        return false;
    const command = words[0];
    if ((!hasRootSeparator(argv) && (command in fixedCommandPositionals || authServices.has(command) || command in removedCommandNames)) || isSourceUrl(command))
        return false;
...
function outputHelpCommandPath(argv) {
    const words = positionalWords(argv);
    if (words === null || words.length === 0)
        return null;
...
function renderInvalidOutputFormat(argv) {
...
function debugModes(argv) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "--debug") {
            ...
            else if (value !== undefined && !value.startsWith("-") && !isDebugTarget(value))
                modes.push(value);
        }
        else if (arg.startsWith("--debug="))
            modes.push(arg.slice("--debug=".length));
    }
    return modes;
}
function renderInvalidDebugMode(argv) {
...
function initBackendValues(argv) {
    const values = [];
    const words = positionalWords(argv);
    if (words === null || words[0] !== "init")
        return values;
    const args = argsBeforeSeparator(argv);
    for (let index = 0; index < args.length; index += 1) {
        const arg = args[index];
        if (arg === "--backend") {
            ...
            values.push(arg.slice("--backend=".length));
    }
    return values;
}
function renderInvalidInitBackend(argv) {
    if (hasHelpOrVersion(argv))
        return false;
...
function hasInvalidGlobalControl(argv) {
...
export async function runWireCli(root, argv, currentDirectory) {
